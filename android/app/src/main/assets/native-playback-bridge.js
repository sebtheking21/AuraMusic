(function(){
  ...
  function isNativeCandidate(url){
    ...
    return u.indexOf('http://') === 0 || u.indexOf('https://') === 0 || u.indexOf('file://') === 0 || u.indexOf('content://') === 0;
  }
  ...
  function sendPlay(media){
    ...
    if (src === lastSrc && nativeApi.playPause) {
      try { nativeApi.playPause(); } catch(e) {}
      return;
    }
    lastSrc = src;
    var info = currentInfo(media);
    try { nativeApi.play(src, info.title, info.artist, info.art); } catch(e) {}
    // The native Media3 player owns the audio so Android can keep it alive after the Activity is hidden.
  ...
  function watch(media){
    ...
    media.addEventListener('loadedmetadata', function(){
      if (media.autoplay && !media.paused) sendPlay(media);
    });
  }
