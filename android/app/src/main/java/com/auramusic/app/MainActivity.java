...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        Window window=getWindow(); window.setStatusBarColor(0xFF000000); window.setNavigationBarColor(0xFF000000);
        ...
        webView=new WebView(this); webView.setBackgroundColor(0xFF000000);
        ...
        int id=getResources().getIdentifier("status_bar_height","dimen","android"); int h=id>0?getResources().getDimensionPixelSize(id):dp(24);
        webView.setPadding(0,h,0,0); webView.setClipToPadding(false); setContentView(webView);
        webView.addJavascriptInterface(new NativeMediaBridge(), "AndroidAura");
        WebSettings settings=webView.getSettings(); settings.setJavaScriptEnabled(true); settings.setDomStorageEnabled(true); settings.setDatabaseEnabled(true);
        settings.setMediaPlaybackRequiresUserGesture(false); settings.setBuiltInZoomControls(false); settings.setDisplayZoomControls(false); settings.setSupportZoom(false);
        settings.setLoadWithOverviewMode(false); settings.setUseWideViewPort(false); settings.setMixedContentMode(WebSettings.MIXED_CONTENT_NEVER_ALLOW); settings.setCacheMode(WebSettings.LOAD_DEFAULT);
        CookieManager.getInstance().setAcceptCookie(true); CookieManager.getInstance().setAcceptThirdPartyCookies(webView,true);
        webView.setWebChromeClient(new WebChromeClient(){@Override public boolean onShowFileChooser(WebView view,ValueCallback<Uri[]> callback,FileChooserParams params){
            if(filePathCallback!=null)filePathCallback.onReceiveValue(null); filePathCallback=callback; try{startActivityForResult(params.createIntent(),FILE_CHOOSER_REQUEST);}catch(Exception e){filePathCallback=null;Toast.makeText(MainActivity.this,"Unable to open file picker",Toast.LENGTH_SHORT).show();return false;} return true;}});
        webView.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView view,String url){super.onPageFinished(view,url);injectMobileUi(view);}@Override public boolean shouldOverrideUrlLoading(WebView view,WebResourceRequest request){return false;}});
        requestNotificationPermission(); startPlaybackService(); webView.loadUrl(AURA_URL);
    }
    ...
    private void sendPlayback(String action,String url,String title,String artist,String art){
        Intent intent=new Intent(this,PlaybackService.class); intent.setAction(action);
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){super.onActivityResult(requestCode,resultCode,data);if(requestCode==FILE_CHOOSER_REQUEST){if(filePathCallback==null)return;Uri[] results=null;if(resultCode==RESULT_OK&&data!=null){Uri uri=data.getData();if(uri!=null)results=new Uri[]{uri};}filePathCallback.onReceiveValue(results);filePathCallback=null;}}
    ...
    private String readAsset(String name){try(InputStream input=getAssets().open(name)){byte[] bytes=new byte[input.available()];int offset=0,read;while(offset<bytes.length&&(read=input.read(bytes,offset,bytes.length-offset))>0)offset+=read;return new String(bytes,0,offset,StandardCharsets.UTF_8);}catch(IOException e){return "";}}
    private void injectMobileUi(WebView view){
        String css=readAsset("mobile.css"),js=readAsset("mobile.js"),transfer=readAsset("playlist-transfer.js"),fixCss=readAsset("mobile-player-fix.css"),fixJs=readAsset("mobile-player-fix.js"),nativeJs=readAsset("native-playback-bridge.js");
        if(css.isEmpty()&&js.isEmpty()&&transfer.isEmpty()&&fixCss.isEmpty()&&fixJs.isEmpty()&&nativeJs.isEmpty())return;
        String css64=Base64.encodeToString(css.getBytes(StandardCharsets.UTF_8),Base64.NO_WRAP),js64=Base64.encodeToString(js.getBytes(StandardCharsets.UTF_8),Base64.NO_WRAP),transfer64=Base64.encodeToString(transfer.getBytes(StandardCharsets.UTF_8),Base64.NO_WRAP),fixCss64=Base64.encodeToString(fixCss.getBytes(StandardCharsets.UTF_8),Base64.NO_WRAP),fixJs64=Base64.encodeToString(fixJs.getBytes(StandardCharsets.UTF_8),Base64.NO_WRAP),native64=Base64.encodeToString(nativeJs.getBytes(StandardCharsets.UTF_8),Base64.NO_WRAP);
    ...
    @Override protected void onDestroy(){if(filePathCallback!=null){filePathCallback.onReceiveValue(null);filePathCallback=null;}if(webView!=null){webView=null;}super.onDestroy();}
